...
const Storage = {
  async get() {
    const stored = await ext.storage.local.get(["gridtabs_data"]);
    if (stored && stored.gridtabs_data) {
      const migrated = migrate(stored.gridtabs_data);
      const changed = !Array.isArray(stored.gridtabs_data.sheets)
        || stored.gridtabs_data.sheets.some(s => Array.isArray(s.tabs));
      if (changed) await ext.storage.local.set({ gridtabs_data: migrated });
      return mergeWithDefaults(migrated);
    }
    const fresh = mergeWithDefaults(defaultData());
    await ext.storage.local.set({ gridtabs_data: fresh });
    return fresh;
  },
  async set(data) {
    await ext.storage.local.set({ gridtabs_data: data });
  },
  ...
  onChanged(cb) {
    ext.storage.onChanged.addListener((changes, area) => {
      if (area === "local" && changes.gridtabs_data) {
        cb(changes.gridtabs_data.newValue);
      }
    });
  }
};
