...
(function (root, factory) {
  ...
  // Ключ «сегодня» для почасового прогноза. Берём дату ПЕРВОГО часа прогноза,
  // а не текущее время устройства: прогноз строится в background от Date.now()
  // и кэшируется, поэтому между построением и рендером (в городах с экстремальными
  // поясами, Тараве UTC+12 / Гонолулу UTC-10) дата может смениться. Если считать
  // «сегодня» от new Date() при рендере — первый час (уже вчерашний по городу)
  // подсвечивается «сегодня» через idx===0, а весь новый день — через
  // совпадение с todayKey => два «Сегодня» и синий следующий день.
  function todayHourlyKey(hourly) {
    if (Array.isArray(hourly) && hourly[0] && hourly[0].date) return hourly[0].date;
    return null;
  }
