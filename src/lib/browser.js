...
(function () {
  ...
  function bookmarksGetTree() {
    return new Promise((resolve, reject) => {
      try {
        if (!root.bookmarks || typeof root.bookmarks.getTree !== "function") {
          reject(new Error("bookmarks API unavailable"));
          return;
        }
        // Firefox: browser.bookmarks.getTree() returns a Promise directly.
        if (typeof browser !== "undefined" && browser && browser.runtime === root &&
            root.bookmarks.getTree.length <= 0) {
          Promise.resolve(root.bookmarks.getTree()).then(resolve, reject);
          return;
        }
        // Chromium: wrap the callback API in a Promise.
        root.bookmarks.getTree((tree) => {
          const err = root.runtime && root.runtime.lastError;
          if (err) reject(new Error(err.message || String(err)));
          else resolve(tree || []);
        });
